...
def get_full_stats(user_id):
    ...
    data = []
    for deck in decks:
        data.append((deck[1], db.session.execute("""SELECT u.username, COUNT(*), COALESCE(SUM(a.result),0) FROM answers a, cards c, users u WHERE c.deck_id=:deck_id AND a.card_id=c.id AND u.id=a.user_id GROUP BY u.id, u.username ORDER BY u.username""", {"deck_id": deck[0]}).fetchall()))
    return data